...
////      Author             : Costin S
////
//// Originally taken from   : https://code.google.com/archive/p/self-balancing-avl-tree/
////
/////////////////////////////////////////////////////////////////////
...
//// 
//// When concat and split operations are defined, the code defines stored both the Balance and the Height 
////     in each node which is obviously not necessary. 
//// To reduce space, you can do one of two things:
////      1. The simplest change is to store both Balance and Height in one integer. Balance field needs only 
////          2 bits which lefts 30 bits for the Height field. A tree with a HEIGHT > 2^30 (2 to the power of 30) 
////          is very unlikely you will ever build.
////      2. Simple enough to modify and remove Balance field. Concat and Split were added as an afterthought 
////          after the implementation was already done using a Balance field.
...
namespace AlgorithmSamples.BinaryTreeBalanced.AVLTree
{
    ...
    public class AutoBalancedAvlTree<T>
    {
        #region Fields
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AutoBalancedAvlTree{T}"/> class.
        /// </summary>
        public AutoBalancedAvlTree()
        {
        ...
        /// <param name="comparer">The comparer class to be used to compare the item of the type <see cref="T"/>. 
        /// Omit or use the null value to use the default comparer for the type T</param>
        public AutoBalancedAvlTree(IEnumerable<T> elems, IComparer<T> comparer = null) : base() {
            this.comparer = comparer ?? GetComparer();
        ...
        /// <remarks>
        /// Complexity: 
        ///     Assuming height(node1) > height(node2), our procedure runs in height(node1) + height(node2) 
        ///         i.e. O(log(n1)) + O(log(n2)) due to the two calls to findMin/deleteMin (or findMax, deleteMax respectively). 
        ...
        /// </remarks>
        public AutoBalancedAvlTree<T> Concat(AutoBalancedAvlTree<T> other)
        {
            if (other == null)
            {
                return this;
            ...
            if (root != null)
            {
                return new AutoBalancedAvlTree<T>() { Root = root };
            }
        ...
        /// <returns>a boolean indicating success or failure</returns>
        public bool Split(T value, SplitOperationMode mode, out AutoBalancedAvlTree<T> splitLeftTree, out AutoBalancedAvlTree<T> splitRightTree)
        {
            splitLeftTree = null;
            splitRightTree = null;
            ...
            if (wasFound)
            {
                splitLeftTree = new AutoBalancedAvlTree<T>() { Root = splitLeftRoot };
                splitRightTree = new AutoBalancedAvlTree<T>() { Root = splitRightRoot };
            }
